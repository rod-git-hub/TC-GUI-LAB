...
# (tab id, output name, window size, theme, JS run before capture[, role])
# role defaults to "admin"; "user" renders what a non-admin operator sees.
SHOTS = [
...
def build_pages(workdir):
    ...
    for role in sorted({(shot[5] if len(shot) > 5 else "admin") for shot in SHOTS}):
        ...
        previews[role] = (ROOT / "_preview.html").read_text()

    for shot in SHOTS:
        tab, name, _size, theme, extra = shot[:5]
        preview = previews[shot[5] if len(shot) > 5 else "admin"]
        if tab == "login":
            import re
